...
class Dashboard_Controller extends Auth_Controller {
 ...
 if ($error_conter==0 )  {
  ...
  $html = '<tr id="temp_'.$data['item_location_type'].'" class="old_location" data-loc="'.$data['item_location_type'].'" data-address="'.$address_slug.'"><td class="panel-heading" colspan="7"><span class="locaton_type"><b>'.$loc.'</b>&nbsp;&nbsp;</span><span class="category"><b>'.$data['item_category'].'</b></span></td></tr>';
...
public function insert_disable_location(){
    ...
    if($result) {
      ...
      $html = '<tr id="temp_'.$data['item_location_type'].'" class="old_location" data-loc="'.$data['item_location_type'].'" data-address="'.$address_slug.'"><td class="panel-heading" colspan="7"><span class="locaton_type"><b>'.$loc.'</b>&nbsp;&nbsp;</span><span class="category"><b>'.$data['item_category'].'</b></span><div class="format"><span class="remove_attribute_place" id="'.$item_id.'"><i class="fa fa-minus-circle"></i></span><span class="edit_attribute_group" data-id="'.$item_id.'"><i class="fa fa-pencil" aria-hidden="true"></i>
      </span></div></td></tr>';
      $checkbox_html = '<tr class="checkbox_tr" data-oldloc="'.$data['item_location_type'].'" data-oldaddress="'.$address_slug.'"><td colspan="6" data-ent="entire-location">Entire Location and all chometz on premises</td><input type="hidden" name="item_ids[]" value="'.$item_id.'"/></tr>';
    }else {
...
echo json_encode(array('success'=>$success,'error'=>$error,'item_id'=>$item_id/*,'item'=>$item*/));
...

  {
   ...
   $item_id = $this->input->post('item_id');
   $tbody_address = $this->input->post('json_address');
   $tbody_loc = $this->input->post('json_loc');
...

    {
     ...
     if(!empty($copyitem_id)) {
      ...
      $itemids = $this->Common_model->getResultArray(array('item_id'=>$copyitem_id),'*','order_item');
       ...
       $html = '<tr id="temp_'.$data['item_location_type'].'" class="old_location" data-loc="'.$data['item_location_type'].'" data-address="'.$address_slug.'"><td class="panel-heading" colspan="7"><span class="locaton_type"><b>'.$data['item_location_type'].'</b>&nbsp;&nbsp;</span><span class="category"><b>'.$address_slug.'</b></span></td></tr>';
...
// Copy aboce method
...

  {
    ...
    // refresh div data
     $address = $this->Common_model->item_address($order_id[0]['order_id']);
       $small = array();
       $explode =  array();
       $i =0;
       $small['order'] = $this->Common_model->getResultArray(array('ord_id'=>$order_id[0]['order_id']),'*','order_master');
      ...
      $small['addressandlocation'] = $explode;
       ...
       foreach ($explode as $key => $value) {
          $item_detail = $this->Common_model->wherein_item_location($value['address'],$value['location_type'],$order_id[0]['order_id']);
          $image_data= array();
...
public function update_tr(){
  ...
  if($result){
    ...
    $success = "Update row succesfully";
    $error = "";
    echo json_encode(array('success'=>$success,'error'=>$error,'item'=>$item_id,'body'=>$itemrow,'head'=>$html));
  }else {
    $success = "";
    $error = "Not updated";
    echo json_encode(array('success'=>$success,'error'=>$error,'item'=>$item_id,'body'=>$itemrow,'head'=>$html));
...

  {
    ...
    $success=$error='';
    $item_id = $this->input->post('item_id');
    $item_details = $this->Common_model->getResultArray(array('item_id'=>$item_id),'item_id,item_category,item_location_type','order_item');
  ...
  //   print_r($item_details);exit;
   ...
   echo $this->load->view('dashboard/edit_tr', $data, true);
  ...
  */
...

   {
    ...
    // echo "dasdsadsdsadsad";exit;
      ...
      echo $data['signature_attorney'];exit;
 ...
 */
  ...
  /* ORDER ITEMS AND ORDER IMAGES*/
    ...
    foreach ($order_items as $key => $value) {
     ...
     $copy['disable-location'] = $value['disable-location'];
    // exit;
    ...
    /* ORDER ITEMS AND ORDER IMAGES*/
      ...
      redirect('duplicate_order/'.$neworder_id,'refresh');
